...
| `223027` | 24-bit, 108 759 → 10 875.9 if ×0.1 km | `[B4:B6]/10` | only read in Aug | odometer **candidate** — Rob to confirm against dash (vs A001 f0 = 40 809) |
...
Alternative: WiCAN Pro **Custom Filters** can sniff broadcast frame `0xC10A040`
(MSG31B_EVCU) which OVMS decodes as `d[0]` = estimated range km and `d[1]>>1` =
**display SoC** — the dash number directly, no formula
([OVMS vehicle_fiat500e.cpp](https://github.com/openvehicles/Open-Vehicle-Monitoring-System-3/blob/master/vehicle/OVMS.V3/components/vehicle_fiat500/src/vehicle_fiat500e.cpp)).
Only present while the C-CAN is awake, same as the UDS reads.
...
| odometer (candidate) | 22A001 | `(B5*65536+B6*256+B7)/10` | km | distance — confirm against dash first |
| odometer (candidate 2) | 223027, init `ATSHDA42F1;` | `[B4:B6]/10` | km | — |
